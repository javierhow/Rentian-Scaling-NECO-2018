function network = import_tfs_network(filename, startRow, endRow)
...
list_tfs = dataArray{:, 1};

%% Prepare in format used by code
count = length(list_tfs)/2;
network = cell(count,2);
t = 1;
s = 1;
while t < length(list_tfs)
    network(s,1) = upper(list_tfs(t));
    network(s,2) = upper(list_tfs(t+1));
    t = t + 2;
    s = s + 1;
end
end